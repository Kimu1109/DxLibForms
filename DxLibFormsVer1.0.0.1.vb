...
Public Class DxLibButton
    ...
    Public Sub Draw()
        ...
        TextHeight = SingleTextHeight * TextArr.Length
...
Public Class DxLibListBox
    ...
    Public Sub Draw()
        ...
        If UBound(ItemStr) <= ValueAt Then
            ValueAt = UBound(ItemStr)
        End If

        SingleTextHeight = GetFontSizeToHandle(FontHandle)
        If AutoSize Then
            Height = SingleTextHeight * ItemStr.Length
        Else
        ...
        If ItemStr.Length = 0 Or Height / SingleTextHeight >= ItemStr.Length Then
            ScrollY = 0
            ScrollHeight = Height
        Else
        ...
        If posx >= X + Width - 10 And posx <= X + Width + 10 Then
            If posy > Y And posy < Y + Height Then
                If input <> 0 Then
                    If (posy - Y) / Height * ItemStr.Length <= ItemStr.Length - (Height / SingleTextHeight) Then
...
Public Class DxLibLabel
    ...
    Public Sub Draw()
        ...
        SingleTextHeight = GetFontSizeToHandle(FontHandle)
        TextHeight = SingleTextHeight * Text.Split(vbCrLf).Length
...
Public Class DxLibTextBox
    ...
    ''' </summary>
    ''' <param name="sender">DxLibTextBoxのオブジェクト</param>
    Public Event FinishChange(sender As Object)

    Public ChangedText_var As Boolean
    Public MouseInput_var As Boolean
    Public MouseEndInput_var As Boolean
    Public InArea_var As Boolean
    Public FinishChange_var As Boolean
    ...
    Public Sub New(Optional X_int As Integer = 0,
                   ...
                   Optional BackGroundTrans_bool As Boolean = False,
                   Optional Visible_bool As Boolean = True,
                   Optional MultiLine_bool As Boolean = False)
    ...
    Public Sub Draw()
        ...
        If Text <> BufText Then
            RaiseEvent ChangedText(Me, Text)
            ChangedText_var = True
        End If
        BufText = Text
        ...
        Else
            ...
            If input <> 0 Then
                PushButton = False
            End If
            If input <> 0 And CheckKeyInput(Keys) = 0 Then
                DeleteKeyInput(Keys)
                RaiseEvent FinishChange(Me)
                FinishChange_var = True
            End If
        End If
